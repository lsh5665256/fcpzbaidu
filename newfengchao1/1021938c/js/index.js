...
$('#down-apk').on('click',function () {
    ...
    if (isAndroid) {
        if (suiji > 0.5) {
            ...
            window.location.href = 'https://downpack.baidu.com/baidumap_AndroidPhone_1021938c.apk';
        } else {
            // 否则去走商店的逻辑
            ...
            $.ajax({
                ...
                success: (res) => {
                    ...
                    $.ajax({
                        type: 'GET',
                        async: false,
                        url: 'http://integralwall.baidu.com/integralwall/interact/landingrecoder?partner=fengchao&event_type=' + event_type + '&os=' + xitong + '&browser='+liulanqi + '&model=' + phoneInfo + '&loc_city=' + city + '&plan_id=' + objData.plan_id +'&unit_id=' + objData.unit_id + '&idea_id=' + objData.idea_id + '&word_id=' + objData.word_id+ '&flag_download=' + flag_download + '&flag_view=' + flag_view,
                        dataType: 'jsonp',
                        jsonpCallback: 'callback',
                        success: (res) => {
                            console.log(190)
